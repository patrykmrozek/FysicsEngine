...
int main() {
    ...
    fyWorld* world = fy_world_create(&config);
    if (!world) {
        printf("Error creating world\n");
    }
    fyBody* body = fy_world_create_body(world, 50.0f);
    if (!body) {
        printf("Error creating body\n");
    }
    
    fyVec2 body_pos = fy_body_get_position(body);
    printf("Initial Body Pos: {%f, %f}\n", body_pos.x, body_pos.y);

    fy_body_set_position(body, 100.0f, 100.0f);
    fyVec2 new_body_pos = fy_body_get_position(body);
    printf("New Body Pos: {%f, %f}\n", new_body_pos.x, new_body_pos.y);
    

    // ===========================

    fyBody* body1 = fy_world_create_body(world, 10.0f);
    fyBody* body2 = fy_world_create_body(world, 5.0f);

    fy_body_set_position(body1, 100.0f, 100.0f);
    fy_body_set_position(body2, 150.0f, 50.0f);

    fyVec2 body1_pos_init = fy_body_get_position(body1);
    fyVec2 body2_pos_init = fy_body_get_position(body2);
    ...
    for (int i = 0; i < 60; i++) {
        fy_world_step(world, 1.0f / 60.0f);
    }
    //body's x position should remain the same,
    //while the y position should be altered due to gravity
    fyVec2 body1_pos_update = fy_body_get_position(body1);
    fyVec2 body2_pos_update = fy_body_get_position(body2);
